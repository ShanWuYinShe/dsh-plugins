...
* **Seatbelt 分隔符位置漂移时放弃重建**：官方 argv 为 `[sandbox-exec, -p, <profile>, --, inner]`，此前只校验分隔符存在、不校验位置——官方若在 profile 与 `--` 之间新增参数，整表重建会把它静默丢掉，沙盒比官方更宽；漂移自检只比对 profile 文本，检不出这种形态。现在分隔符不在位置 3 时告警一次并保持官方 argv 原样（fail-closed，bash 侧额外根失效、fs 侧放行不受影响）。
* **危险根预览的 `$` 序列注入**：设置卡的预览文本此前用字符串替换插入用户输入的原始行，`$&`/`` $` ``/`$'` 等特殊序列会把模板前后文拼进预览，显示的路径与实际输入不符；改函数替换。

### 测试

* Landlock runner 路径补平台 mock 用例（`--rw` 插入位置、存在性过滤、无分隔符 fail-closed）——三条 runner 路径此前测了两条。
* 多挂载点引用计数用例：双挂载先后 dispose，第一次退出包装必须保留、最后一次才还原（loader 热切换的设计目标）。
