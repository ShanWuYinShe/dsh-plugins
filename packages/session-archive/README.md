...
- **筛选与排序**：工具条输入框按标题、工作区路径或会话 ID 子串即时筛选
  （前端过滤，不发请求）；排序在最近修改/体积/标题间循环切换。筛选与排序
  在页面重载后保留（sessionStorage）。
- 工具条提供全选与已选计数（全选只作用于当前筛选结果）；列表分页加载避免
  大归档量全量渲染；侧边栏徽标实时显示归档数量（订阅宿主归档集合，轻量
  轮询兜底）。
