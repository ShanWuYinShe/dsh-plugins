 ...
 *
 * Ported from corrinehu/dsh-workbuddy-connect `src/probe.ts` (MIT). The order
 * matters and is not an optimization:
 *
 ...
 *
 * Verified live 2026-09-15: CN `deepseek-v4-pro` refuses the sentinel
 * (400 + `extError.code: invalid_reasoning_effort`) while `glm-5.2` streams
 * 200 on it; international `deepseek-v4.1-flash` refuses identically and
 * `kimi-k3` streams. The 400 body shape (`extError.code`) is what the probe
 * keys on, not the top-level numeric code.
 *
 * @module dsh-any-connect/probe
 */
...
/**
 * The upstream's "this effort value is not supported" code. It is *not*
 * treated as a permanent protocol promise: anything unrecognized degrades to
 * `unknown` rather than to a capability conclusion.
 */
