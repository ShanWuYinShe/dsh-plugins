 ...
 *
 * Ported from corrinehu/dsh-workbuddy-connect `src/app-version.ts` (MIT).
 *
 * The App-shaped catalog is served only to a User-Agent carrying the product
 * name: the terse `WorkBuddyAI/<v>` form (no space) returns the full App
 * document, while the space form is rejected (HTTP 400) and the CLI form gets
 * a smaller document without `modelPromotions` — all three measured live on
 * 2026-09-15.
 *
 ...
 *
 * @module dsh-any-connect/app-version
 */
 ...
 *
 * The gateway ignores the version number when splitting the UA (measured
 * 2026-09-11 upstream: several versions all return the same document), so
 * this constant is a shape requirement rather than a currency claim. It is
 * *not* used to infer anything about model capabilities.
 */
 ...
 * document; the space form is rejected with 400. Throws on an invalid version
