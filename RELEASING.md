...
|---|---|---|---|---|
| `main` | DSH 稳定线（当前值以 `pnpm run dsh-status` 为准） | npm `latest` | 纯 semver（如 `0.10.3`） | `latest` |
...
> **宿主侧的「正式版」= 被发成 `latest` 的 rc 版本**（如 `0.1.5-rc.1` 就是
> 0.1.5 线的正式版）。dsh **从不发布无后缀的纯 `X.Y.Z`**，`0.1.5-rc.*` 之后
> 直接进 `0.1.6-alpha.1`；因此不要等「dsh 0.1.6 正式版」这种不存在的版本，
> 判断当前线是否终结只看 `latest` 指向哪个 rc。注意 `next` dist-tag 不参与
> 本仓的任何判定（脚本只读 `latest` 与 `-alpha` 序列）。
